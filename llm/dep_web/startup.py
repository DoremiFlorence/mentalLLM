...
max_number = {
    "category_1": 5, "category_2": 6, "category_3": 5, "category_4": 2, "category_5": 2
}
now_number = {
...
def explain_score(score):
    ...
    elif standar >= 0.5 and standar < 0.59:
        return "您可能有轻度抑郁情绪，请进行自我调节，或寻求他人支持帮助"
    elif score >= 0.6 and score < 0.69:
        return "您最近可能处于中度抑郁状态，请寻找心理专家咨询"
    elif score > 0.7:
        return "您最近可能处于重度抑郁状态，请尽快找心理专家咨询"
...
def chat_api():
    ...
    if check_max_number_reached(now_number, max_number):
        total_score += 20
        explain = explain_score(total_score)
        return jsonify(
            {"reply": reply, "next_question": None, "total_score": total_score, "explain": explain, "finished": True})

    return jsonify(
        {"reply": reply, "next_question": now_ques, "total_score": total_score, "finished": False})
